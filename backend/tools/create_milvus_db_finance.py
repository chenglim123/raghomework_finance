...
db_path = "backend/db/finance_bge_m3.db"
...
logging.info(f"Search result for 'Attestation': {search_result}")

# 查询所有匹配的实体
query_result = client.query(
    collection_name=collection_name,
    filter="concept_name == 'Attestation'",
    output_fields=["concept_name", 
                #    "synonyms", 
                #    "concept_class_id", 
                   ],
...
logging.info(f"Query result for concept_name == 'Attestation': {query_result}")
if client:
    try:
        client.close()
        logging.info("Successfully closed Milvus connection")
    except Exception as e:
